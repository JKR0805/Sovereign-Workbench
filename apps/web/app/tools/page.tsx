...
import { AstGuardResult } from '../../lib/types';
import {
  Wrench,
  ShieldAlert,
  Terminal,
...
print(calculate_pressure(342))`
  ...
  const handleTestSandboxEgress = () => {
    setSandboxOutput(
      `[SANDBOX RUNNER] Spawning ephemeral container (image: vajra-sandbox:py311, network: none)...
...
[SANDBOX SENTINEL] Confirmed: Container has zero virtual network interfaces (isolated kernel netns).`
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto flex flex-col gap-6">
      ...
      {/* Sandbox Isolation Policy Banner matching FRONTEND_SPECIFICATION.md Section 4.9 */}
      <div className="bg-bg-panel border border-border rounded-lg p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded bg-bg-elevated border border-border flex items-center justify-center text-ok flex-shrink-0">
            <ShieldAlert className="w-5 h-5 text-ok" />
          </div>
          <div>
            <h2 className="text-xs font-bold text-text-primary font-mono uppercase">
              Docker Sandbox Isolation Parameters
            </h2>
            <div className="text-[11px] font-mono text-text-tertiary flex flex-wrap gap-3 mt-0.5">
              <span>network_disabled: <strong>true (--network=none)</strong></span>
              <span>·</span>
              <span>memory_limit: <strong>1.0 GB</strong></span>
              <span>·</span>
              <span>cpu_limit: <strong>2 Cores</strong></span>
              <span>·</span>
              <span>rootfs: <strong>read-only</strong></span>
            </div>
          </div>
        </div>
        ...
        <button
          ...
          className="px-3 py-1.5 rounded bg-bg-elevated border border-border hover:border-error text-error text-xs font-mono transition-colors self-start sm:self-auto"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-bg-panel border border-border rounded-lg p-5 flex flex-col gap-3">
          <h2 className="text-xs font-bold text-text-primary uppercase tracking-wider font-mono">
            Registered Agent Tools ({MOCK_TOOLS.length})
          </h2>
          ...
          <div className="flex flex-col gap-3 overflow-y-auto max-h-[500px]">
            {MOCK_TOOLS.map((tool) => (
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="font-mono font-bold text-accent">{tool.name}</span>
                  <span className="font-mono text-[10px] px-1.5 py-0.2 rounded bg-bg-base border border-border text-ok">
                ...
                <div className="pt-2 border-t border-border/60">
                  ...
                  <pre className="p-2 rounded bg-bg-base border border-border font-mono text-[10px] text-text-secondary overflow-x-auto">
