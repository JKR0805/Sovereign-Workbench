...
@pytest.mark.asyncio
async def test_runtimes_endpoints(async_client: httpx.AsyncClient) -> None:
    """Verify runtimes enumeration and probing endpoints."""
    resp = await async_client.get("/api/runtimes")
    assert resp.status_code == 200
    assert isinstance(resp.json(), list)

    get_resp = await async_client.get("/api/runtimes/ollama")
    assert get_resp.status_code in (200, 404)

    avail_resp = await async_client.get("/api/runtimes/ollama/available")
    assert avail_resp.status_code in (200, 404)

    probe_resp = await async_client.post("/api/runtimes/ollama/probe", json={})
    assert probe_resp.status_code in (200, 404)


@pytest.mark.asyncio
async def test_models_endpoints(async_client: httpx.AsyncClient) -> None:
    """Verify model registry, residency tracking, and health refresh."""
    list_resp = await async_client.get("/api/models")
    assert list_resp.status_code == 200
    models = list_resp.json()
    assert isinstance(models, list)

    residency_resp = await async_client.get("/api/models/residency")
    assert residency_resp.status_code == 200
    assert "entries" in residency_resp.json()

    if models:
        first_id = models[0]["id"]
        get_resp = await async_client.get(f"/api/models/{first_id}")
        assert get_resp.status_code == 200
        assert get_resp.json()["id"] == first_id

        refresh_resp = await async_client.post(f"/api/models/{first_id}/refresh", json={})
        assert refresh_resp.status_code == 200


@pytest.mark.asyncio
async def test_routing_endpoints(async_client: httpx.AsyncClient) -> None:
    """Verify routing capability profiles and simulation engine."""
    caps_resp = await async_client.get("/api/routing/capabilities")
    assert caps_resp.status_code == 200

    policies_resp = await async_client.get("/api/routing/policies")
    ...
    sim_resp = await async_client.post("/api/routing/simulate", json=sim_payload)
    assert sim_resp.status_code == 200


@pytest.mark.asyncio
async def test_knowledge_and_rag_endpoints(async_client: httpx.AsyncClient) -> None:
    ...
    doc_resp = await async_client.get(f"/api/knowledge/documents/{doc_id}")
...
@pytest.mark.asyncio
async def test_tools_endpoints(async_client: httpx.AsyncClient) -> None:
    """Verify tool discovery and schema inspection."""
    tools_resp = await async_client.get("/api/tools")
    ...
    single_tool_resp = await async_client.get(f"/api/tools/{tool_name}")
    assert single_tool_resp.status_code == 200
    assert single_tool_resp.json()["name"] == tool_name


@pytest.mark.asyncio
async def test_sandbox_endpoints(async_client: httpx.AsyncClient) -> None:
    """Verify sandbox runner status, execution policies, and AST guard analysis."""
    status_resp = await async_client.get("/api/sandbox/status")
    assert status_resp.status_code == 200

    policy_resp = await async_client.get("/api/sandbox/policy")
    assert policy_resp.status_code == 200

    # Benign code should be permitted
    benign_resp = await async_client.post(
        "/api/sandbox/guard",
...
@pytest.mark.asyncio
async def test_network_and_sovereignty_endpoints(async_client: httpx.AsyncClient) -> None:
    """Verify network sentinel tables, events, and sovereignty self-audit."""
    snap_resp = await async_client.get("/api/network/snapshot")
    assert snap_resp.status_code == 200
    assert "connections" in snap_resp.json()

    events_resp = await async_client.get("/api/network/events")
    assert events_resp.status_code == 200
    assert isinstance(events_resp.json(), list)

    nft_resp = await async_client.get("/api/network/nft/status")
    assert nft_resp.status_code == 200

    audit_resp = await async_client.get("/api/network/selfaudit")
    assert audit_resp.status_code == 200

    ruleset_resp = await async_client.get("/api/network/ruleset")
    assert ruleset_resp.status_code in (200, 503)


@pytest.mark.asyncio
async def test_audit_endpoints(async_client: httpx.AsyncClient) -> None:
    """Verify audit event types, persistent event queries, and export contract."""
    types_resp = await async_client.get("/api/audit/event-types")
    assert types_resp.status_code == 200
    assert len(types_resp.json()) > 0

    events_resp = await async_client.get("/api/audit/events")
    assert events_resp.status_code == 200
    assert "items" in events_resp.json()

    # 501 Not Implemented expected for Phase 2 signed export bundle
    export_resp = await async_client.get("/api/audit/export")
    assert export_resp.status_code in (200, 501)


@pytest.mark.asyncio
async def test_runs_orchestrator_lifecycle(async_client: httpx.AsyncClient) -> None:
    """Verify complete run lifecycle: create, poll status, retrieve steps, and SSE events."""
    list_resp = await async_client.get("/api/runs")
    assert list_resp.status_code == 200
    ...
    create_resp = await async_client.post("/api/runs", json=create_payload)
    ...
    get_resp = await async_client.get(f"/api/runs/{run_id}")
    assert get_resp.status_code == 200
    ...
    artifacts_resp = await async_client.get(f"/api/runs/{run_id}/artifacts")
    assert artifacts_resp.status_code == 200

    sse_resp = await async_client.get(f"/api/runs/{run_id}/events")
